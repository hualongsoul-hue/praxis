...
class ToolPolicy:
    ...
    def __init__(self, config: ToolsConfig) -> None:
        self._allowed_paths = tuple(Path(item).expanduser().resolve() for item in config.allowed_paths)
        self._shell_timeout = config.shell_timeout
        self._default_timeout = config.default_timeout
        self._shell_enabled = config.shell_enabled
        self._network_allowed = config.network_allowed
        self._allow_private_networks = config.allow_private_networks
        self._network_max_response_bytes = config.network_max_response_bytes
        self._environment_allowlist = frozenset(config.shell_environment_allowlist)
    ...
    def network_max_response_bytes(self) -> int:
        return self._network_max_response_bytes
    ...
    def check_path(self, path: str | Path) -> Path:
        ...
        raise ToolPolicyViolationError(
            ...
            details={
                "path": str(resolved),
                "allowed_paths": [str(item) for item in self._allowed_paths],
            },
        )
    ...
    def shell_environment(
        ...
        allowed = _SAFE_ENVIRONMENT | self._environment_allowlist
