...
`praxis doctor` 在密钥存在时会执行真实模型探针；它适合部署诊断，不适合每秒调用的 liveness。
...
配置中的相对路径以 YAML 文件目录为基准。如果从其他工作目录启动后路径不同，先检查是否误把
期望的进程工作目录当成了配置目录。
...
内容块无关。只有至少三个独立的“文本控制成功 → 模态调用成功 → 文本控制成功”真实端点健康窗口
产生一致结果后，才把对应字段设为 `true`。
...
报告包含最近探针时间、延迟、安全错误类别和 required 标记；它不会包含原始异常或凭据。

## 自定义验证器被改写为 ERROR

计算验证器必须返回与注册名称一致的 `verifier_name`、`computational` 类型，并在 PASS 时提供
非空反馈。空验证集合也不会被视为通过。修正适配器契约，不要在 GAV 层伪造 PASS。
