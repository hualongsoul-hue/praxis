...
| 原则 | 说明 |
|------|------|
| **子系统组合** | Harness 由独立子系统组合而成，每个子系统有明确边界和公开接口 |
...
Praxis 不是一个单体应用，而是 **14 个独立子系统的组合体**。每个子系统：
...
**分层规则**：每一层仅允许依赖同层或更低层的子系统，禁止向上依赖。

### 2.3 子系统依赖图

```
S13(子代理协调) ───▶ S11, S12
S12(生命周期)   ───▶ S3, S6, S7, S11, S14
S11(编排循环)   ───▶ S4, S5, S6, S7, S8, S9, S10, S14
                          │
S10(验证引擎)   ───▶ S4, S5
S9 (错误恢复)   ───▶ (无子系统依赖，仅 S1, S2)
S8 (护栏系统)   ───▶ (无子系统依赖，仅 S1, S2)
                          │
S14(技能系统)   ───▶ S3, S5
...
S5 (工具系统)   ───▶ S3
                          │
S4 (模型网关)   ───▶ (无子系统依赖，仅 S1, S2)
                          │
S3 (持久化引擎) ───▶ (无子系统依赖，仅 S1)
S2 (遥测系统)   ───▶ (无子系统依赖，仅 S1)
S1 (配置系统)   ───▶ (无依赖)
```

> **注意**：所有子系统隐式依赖 S1（配置）和 S2（遥测），上图中省略这两个通用依赖以保持清晰。

### 2.4 子系统总览

| 编号 | 子系统 | 包路径 | 层级 | 职责边界 |
|------|--------|--------|------|---------|
| S1 | 配置系统 | `praxis.config` | L0 | 全局配置加载、验证、热更新；所有子系统的配置来源 |
...
| S12 | 生命周期管理 | `praxis.lifecycle` | L4 | 会话初始化/恢复、检查点、跨窗口续接 |
...
配置系统是所有子系统的配置唯一来源。它负责加载、合并、验证和分发配置数据，但不负责任何业务逻辑。
...
| `get_subsystem_config(name)` | 子系统名称 | 对应子系统的配置切片 | 各子系统初始化时 |
...
- 顶层配置对象 `PraxisConfig` 包含每个子系统的配置切片

**F1.2 子系统配置隔离**

- 每个子系统拥有独立的配置 Section，命名空间隔离
- 子系统仅能访问自身配置切片，不可越权访问其他子系统配置
- 配置 Schema 由各子系统自行定义，注册到配置系统

**F1.3 配置验证**

- 启动时全量校验所有配置项，校验失败拒绝启动并报告具体错误
- 支持自定义验证器：子系统可注册跨字段验证规则
- 运行时配置变更同样必须通过验证
...
| 接口 | 输入 | 输出 | 调用方 |
|------|------|------|--------|
| `get_logger(name)` | 模块名 | 结构化 Logger 实例 | 所有子系统 |
| `emit_metric(name, value, tags)` | 指标名、值、标签 | — | 所有子系统 |
...
- 所有日志条目包含：时间戳、级别、子系统名、会话 ID、轮次号、消息、结构化字段
- 日志级别配置化，支持按子系统独立设置
...
| 接口 | 输入 | 输出 | 调用方 |
|------|------|------|--------|
| `save(namespace, key, data)` | 命名空间、键、可序列化数据 | — | S6(记忆)、S12(生命周期) |
| `load(namespace, key)` | 命名空间、键 | 反序列化后的数据或 None | S6(记忆)、S12(生命周期) |
| `delete(namespace, key)` | 命名空间、键 | — | S6(记忆)、S12(生命周期) |
| `list_keys(namespace, prefix?)` | 命名空间、可选前缀 | 键列表 | S6(记忆)、S12(生命周期) |
| `save_checkpoint(session_id, state)` | 会话 ID、状态快照 | 检查点 ID | S12(生命周期) |
| `load_checkpoint(checkpoint_id)` | 检查点 ID | 状态快照 | S12(生命周期) |
...
| `run_dream()` | — | 整理报告 | S12(生命周期)、手动触发 |
| `read_scratchpad(key)` | 草稿键名 | 草稿内容或 None | S7(上下文引擎)、S12(生命周期) |
| `write_scratchpad(key, content)` | 草稿键名、内容 | — | S11(编排循环) 通过工具 |
| `clear_session()` | — | — | S12(生命周期) |
| `export_state()` | — | 可序列化的记忆状态快照 | S12(生命周期) |
| `import_state(snapshot)` | 记忆状态快照 | — | S12(生命周期) |
...
**被依赖**：S11（编排循环）、S12（生命周期）

#### 职责边界

上下文引擎是 Harness 中最关键的子系统之一。它负责在每一步为模型组装最优上下文——决定模型"看到什么"以及"何时看到"。它消费 S5（工具 Schema）、S6（记忆内容）和 S4（Token 计数/压缩摘要），产出可直接送入 LLM 的完整 Prompt。
...
| `export_state()` | — | 可序列化的上下文状态 | S12(生命周期) |
...
**被依赖**：S7（上下文引擎）、S11（编排循环）、S12（生命周期）
...
技能系统不执行工具（由 S5 执行），不控制编排循环（由 S11 控制），不管理上下文组装（由 S7 管理）。它只负责技能的发现、加载、生命周期管理和向上游子系统提供技能内容。

#### 公开接口契约

| 接口 | 输入 | 输出 | 调用方 |
|------|------|------|--------|
| `discover_skills(paths)` | 技能搜索路径列表 | 已发现技能的元数据列表 | 启动时 / S12(生命周期) |
...
错误恢复是纯策略子系统——它接收错误信息，返回恢复策略（重试、降级、中止等）。它维护重试计数和熔断器状态，但不执行具体的恢复动作（由 S11 编排循环执行）。
...
验证引擎负责评估 Agent 产出的质量。它是将演示级 Agent 提升为生产级 Agent 的关键子系统。验证引擎提供三种验证能力（计算型、推理型、视觉），由 S11（编排循环）在适当时机调用。
...
**被依赖**：S12（生命周期）、S13（子代理协调）

#### 职责边界

编排循环是 Harness 的心跳——实现 **Thought → Action → Observation（TAO）** 循环。它是所有子系统的协调中枢，负责按正确顺序调用各子系统完成一个完整的 Agent 轮次。编排循环本身是"哑循环"，所有智能由模型提供，Harness 仅管理轮次流程和子系统协调。

#### 公开接口契约

| 接口 | 输入 | 输出 | 调用方 |
|------|------|------|--------|
| `run(user_message, config?)` | 用户消息、可选运行配置 | `AgentResponse`（最终响应 + 执行元数据） | S12(生命周期)、外部调用者 |
| `run_stream(user_message, config?)` | 同上 | `AsyncIterator[AgentEvent]`（事件流） | S12(生命周期)、外部调用者 |
| `abort()` | — | — | S12(生命周期)、用户中断 |
| `get_state()` | — | 当前循环状态（轮次数、阶段、Token 用量） | S12(生命周期) |
...
### 7.2 S12：生命周期管理（Lifecycle Manager）

**包路径**：`praxis.lifecycle`
**依赖**：S1（配置）、S2（遥测）、S3（持久化）、S6（记忆）、S7（上下文引擎）、S11（编排循环）
**被依赖**：S13（子代理协调）、外部调用者

#### 职责边界

生命周期管理负责 Agent 会话的"诞生"到"终结"全过程——初始化、运行、暂停、恢复、终止。它管理跨上下文窗口的状态续接，使 Agent 能在长期项目中存活。S12 是外部调用者与 Harness 交互的主要入口。
...
**包路径**：`praxis.subagent`
**依赖**：S1（配置）、S2（遥测）、S11（编排循环）、S12（生命周期）
...
- 每个子代理拥有独立的子系统实例集（S4~S11），通过 S12 创建
...
以下场景走查展示子系统间的完整协作流程，验证依赖关系和接口契约的完整性。
...
用户消息
    │
    ▼
S12.run_turn(session, message)               ─── 生命周期入口
    │
...
S12.resume_session("session_abc")              ─── 恢复入口
    ...
    │
    ├─→ 重新初始化 S4, S5, S8, S9, S10, S11, S14 ── 无状态子系统重建
    │
    └─→ 返回恢复后的 Session
         │
         ▼
...
| 8 | 子系统通信 | 直接函数调用，非消息队列 | 单进程部署，避免分布式复杂度；未来可替换为消息总线 | 全局 |
| 9 | 状态持久化 | 检查点式，非事件溯源 | 检查点简单可靠；事件溯源在当前规模下过度工程化 | S3, S12 |
| 10 | 技能知识 vs 工具能力 | Skills 与 Tools 分离，独立子系统 | Skills 提供程序化知识，Tools 提供执行能力；分离后可独立演进、组合复用 | S5, S14 |
...
| 术语 | 定义 | 关联子系统 |
|------|------|-----------|
| **Agent Harness** | 包裹 LLM 的完整软件基础设施，由 14 个子系统组合而成 | 全部 |
...
*Praxis Harness 由 14 个子系统分 5 层组合构建，每个子系统可独立开发、测试和替换。*
